...
<?php
            foreach ($lsBaiDang as $data) {
                if($data['id_account']==Auth::user()->fullname){
            ?>
@csrf
<div id="listphone">
    <a href="{{route('chinh-sua-bai-dang',['id'=>$data['id_post']])}}" class="item">
        </br>
        ...
        <br /><br />
        <button class="buy" style="margin-bottom: 10px">Chỉnh sửa bài viết</button>
